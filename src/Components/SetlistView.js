...
function SetlistView({list}) {
  ...
  let [passedSl, setPassedSl] = useState([])


  useEffect(() => {
    // console.log(passedSl);

    if(list){
      let psl = sessionStorage.getItem('passSetlist')
      
      if(psl){
        let val2 = JSON.parse(psl)
        setPassedSl(val2)
        let y = val2.songArray
        let filter2 = y
          .map(id => list.find(obj => obj.songId === id)) // match IDs in order of x
          .filter(Boolean);
    
        setListingSongs(filter2)  
      }
    }
